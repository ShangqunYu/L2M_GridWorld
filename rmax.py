...
def sampleRooms(ith_house):
    ...
    for ith_room in range(len(rooms)):
        ...
        if rooms[ith_room] == -1:
            ...
            rooms[ith_room] = np.random.choice(args.num_roomtypes)
    return rooms


def sampleObjects(ith_house, rooms=None):
...
def sampleMDPs(ith_house, goal_type, starting_pos, starting_dir):
    ...
    rooms = sampleRooms(0)
    objects_in_rooms = sampleObjects(0, rooms)
    for ith_mdp in range(args.K):
        ...
        pos = (houseLocToObject[ith_house,:,:] == OBJECT_TO_IDX[goal_type])
        imaginingHouse.recreate(objects_in_rooms, rooms, goal_type, pos)
...
for iter in range(eval_num):
    ...
    eval_env_set = [env_set[0], env_set[1], env_set[2], env_set[3], env_set[4], env_set[5]]
    eval_env_index = [0, 1, 2, 3, 4, 5]
            ...
            if ith_house in eval_env_index:
                ...
                for house in range(len(eval_env_set)):
                    ...
                    for episode in range(10):
                        ...
                        houseRoomToType[ith_house, :] = [-1, -1, -1, -1]
                        houseLocToObject[ith_house, :, :] = houseLocToObject[ith_house, :, :]*0 + OBJECT_TO_IDX[env.goal_type]
                        ...
                        reward_set[ith_house + args.num_envs, house * 10 + episode] += e_reward
    ...
    np.save("3rew2new60rmax_{ith}.npy".format(ith=iter), reward_set)