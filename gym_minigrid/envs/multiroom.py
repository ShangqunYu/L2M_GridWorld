...
class MultiRoomEnv(MiniGridEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        # Choose a random number of rooms to generatec
        ...
        color = self._rand_elem(sorted(doorColors))
        self.goal_pos = self.place_obj(Goal(), roomList[-1].top, roomList[-1].size)
        for i in range(len(roomList)):
            if random.random() < 0.5:
                self.place_obj(Ball(), roomList[i].top, roomList[i].size)
            else:
                self.place_obj(Box(color), roomList[i].top, roomList[i].size)
